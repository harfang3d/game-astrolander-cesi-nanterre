...
while not end_game:
	...
	while not end_game and not level_done:
		...
		if raycast_out.node.IsValid() and thrust_left:
			# RAYCAST DEBUG
			# vtx.Clear()
			# vtx.Begin(0).SetPos(ray_start_pos).SetColor0(hg.Color.Red).End()
			# vtx.Begin(1).SetPos(raycast_out.P).SetColor0(hg.Color.Red).End()
			# hg.DrawLines(vid_scene_opaque, vtx, physx_debug_lines_program)
		...
		# else:
		# 	vtx.Clear()
		# 	vtx.Begin(0).SetPos(ray_start_pos).SetColor0(hg.Color.Green).End()
		# 	vtx.Begin(1).SetPos(ray_end_pos).SetColor0(hg.Color.Green).End()
		# 	hg.DrawLines(vid_scene_opaque, vtx, physx_debug_lines_program)
		...
		ray_end_pos = ray_start_pos + ray_dir * 10
		raycast_out = physics.RaycastFirstHit(scene, ray_start_pos, ray_end_pos)
		if raycast_out.node.IsValid() and thrust_right:
			# RAYCAST DEBUG
			# vtx.Clear()
			# vtx.Begin(0).SetPos(ray_start_pos).SetColor0(hg.Color.Red).End()
			# vtx.Begin(1).SetPos(raycast_out.P).SetColor0(hg.Color.Red).End()
		...
		UpdateParticleSystem(engine_particles, render_state_quad_occluded, dtsmooth, cam_rot, view_id_scene_alpha, vtx_layout_particles, texture_smoke)
